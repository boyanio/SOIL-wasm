# SOIL-wasm
This is a clone of Simple OpenGL Image Library from http://lonesock.net/soil.html which hasn't changed since July 7, 2008. I have only done some small modifications in order to compile the library to WebAssembly.

## Build
To build the library for WebAssembly, just run `build-wasm.bat`. You need to download and install Emscripten as described on the official [website](http://kripken.github.io/emscripten-site/docs/getting_started/downloads.html) before that. The build script will generate a *.bc* file that you need to build together with your other sources.
